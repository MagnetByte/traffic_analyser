...
class CmdArgs(object):
    ...
    def main_args(self):
        ...
        lst_args = ['Time Options', 'minute', 'hour', 'day', 'from time',
                    ...
                    'get/post']
        lst_args_format = list(map(lambda x: Col.U + x + Col.ENDC
...
class PrintData(object):
    ...
    def print_date_logs(self, date_logs, start_time, end_time):
        ...
        # avoid repetition of strftime function
    ...
    def print_request(self, request_logs, start_time, end_time, ip_no,request_no, opts):
        """ print all requests related data """

        print()
        self.display_time_range(start_time, end_time, opts, request_no,'request(s)')

        # sort requests per count
        request_logs = sorted(request_logs.items(), key=itemgetter(1), reverse=True)
    ...
    def ip_api(self, ip):
        ...
        # u'message' occurs if ip information cannot be found (i.e., private ip)
    ...
    def general_info(self, opts):
        ...
        if 'default_arg' in self.cmd_args:
            print("✄ Default options detected. The following "
                  "options have been "
    ...
    def print_select(self, print_stuff):
        ...
        for n, (log_file, size) in enumerate(self.data['logs'].items(), 1):
            log = [log_file]
            ...
            if None in self.data['time_period'] or not ip_req_count:
                ...
                if not (self.cmd_args['opts'].ipmatch or
                        self.cmd_args['opts'].rmatch):
                    date_count.clear()
            # reset first/last recorded date
