...
class PrintData(object):
    ...
    def ip_api(self, ip):
        ...
        return(
                    ip_country.decode('utf-8') if isinstance(ip_country, bytes) else ip_country,
                    ip_city.decode('utf-8') if isinstance(ip_city, bytes) else ip_city,
                    ip_isp.decode('utf-8') if isinstance(ip_isp, bytes) else ip_isp,
                    hostname
              )
